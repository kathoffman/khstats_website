...
meds1 <- read.csv("Data/Raw/medslist_10-03-2018.csv", na.strings = c("?"))
dems1 <- read.csv("Data/Raw/demographics_10-03-2018.csv", na.strings = c("","#VALUE!","na"))
...
dems2 <- dems1 %>% mutate(Death = ifelse(Outcome == 2, 1, 0),
                          ...
                          Days.to.first.event = pmin(Days.to.death, Days.to.stroke, Days.to.GIB, Days.to.PT, na.rm=T)) 

names(dems2)
#ggplot(dems2, aes(Days.to.first.outpt.visit)) + geom_histogram(bins=50)
dems2$discharge.to.visit <- dems2$Days.to.first.outpt.visit - dems2$Days.to.IA.discharge
...
First.event <- events %>% replace(is.na(.), Inf) %>% 
  #this will return a 1 for first column if event never occurred, but we want to multiply by NA to convert negated (!) rowSums of logical matrix (!is.na(events))
...
dat <- merge(meds, dems, by="PatientID") %>%
