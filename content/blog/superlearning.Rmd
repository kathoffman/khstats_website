...
- The motivation for this type of "ensembling" is that **a mix of multiple algorithms may fit your data better than any one single algorithm**. For example, a random forest and a penalized regression may each suit your data well, but a mix of both may provide even better predictions.
...
The way I chose to code this was to:

1. make a generic function that combines Step 2 (base learners fit to the training data) and Step 3 (predictions on the validation data)

2. Using `map_dfr()` from the `purrr` package to repeat over all 10 CV folds and then save the results in a new data frame.
...
> **We can now follow this same process with any new observation:**<br>1. Use the fits from each base learner to obtain base learner predictions for the new observation<br>2. Plug those base learner predictions into the metalearner fit
...
Our Superlearner model predicts that an observation with predictors `x1=.5`, `x2=0`, `x3=0`, and `x4=-3` will have an outcome of `r round(predict(sl_fit, newdata = new_pred_df), 3)`. Good to know!
...
You can specify the metalearner with the `method` argument. The default is Non-Negative Least Squares (NNLS).
...
Other packages freely available in `R` that can be used to implement the superlearner algorithm include `sl3` (an update to the older `Superlearner` package), `h2o`, `ml3`, and `caretEnsemble`. I previously wrote a [brief demo](https://www.khstats.com/blog/sl3_demo/sl/) on using `sl3` for an NYC R-Ladies demo.

Python aficionados might find this [blog post](https://machinelearningmastery.com/super-learner-ensemble-in-python/) useful. I have never performed superlearning in Python, but if I had to, I would probably try `h2o` first. H2o is available in many programming languages and their Chief Machine Learning Scientist, Erin Ledell, was an author of the original `SuperLearner` package.

# Coming soon... when prediction is not the end goal

When prediction is not the end goal, superlearning combines well with semi-parametric estimation methods for statistical inference (for example, Targeted Maximum Likelihood Estimation (TMLE)). This allows the use of flexible models and minimal assumptions placed on the distribution of the data.
...
Thank you to Eric Polley, Iván Díaz, Nick Williams and Adam Peterson for very helpful edits and suggestions to this post.
