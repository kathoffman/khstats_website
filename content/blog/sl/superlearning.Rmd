...
kable(broom::tidy(metalrnr_fit), digits=3, caption = "Metalearner regression coefficients") 
```
...
Like most algorithms designed for prediction, the superlearner algorithm does not produce errors, making statistical inference such as confidence intervals and p-values impossible. When prediction is not the end goal, superlearning works well with Targeted Learning methods (for example, Targeted Maximum Likelihood Estimation (TMLE)) for statistical inference. This allows us to utilize flexible models and place minimal assumptions on the distribution of our data.
