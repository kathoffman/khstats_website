...
And then take the mean of those three cross-validated squared error columns to get the CV-MSE.
...
However, we can almost always create an even better prediction algorithm if we use information from *all* of the algorithms' CV predictions to create a new algorithm. This is the key step of the superlearner algorithm: we will use a new learner, a **metalearner**, to take information from all of the base learners and create that new algorithm.

For simplicity, we'll use yet another linear regression as our metalearner. This could be any statistical learning algorithm here (LASSO, non-negative least squares, random forest, etc.). As with any statistical learning algorithm, the choice of algorithm reflects the loss function we want to minimize. 

Whatever metalearner we choose, the predictors will always be the cross-validated predictions from each base learner, and the outcome will always be the true outcome, `y`.
...
This metalearner provides us with the coefficients, or weights, to apply to each of the base learners. We now have our superlearner algorithm.

To implement it on the full data set and obtain predictions, we first need to fit the base learners on the whole data set.
...
Other `R` packages besides `SuperLearner` that can be used to implement the superlearner algorithm include `sl3` (an update to the older `Superlearner` package), `ml3`, `h2o`, and `caretEnsemble`. I wrote a brief tutorial on using `sl3` for an NYC R-Ladies meetup [here](https://www.khstats.com/blog/sl3_demo/sl/).

A python intro to superlearning blog post can be found
[here](https://machinelearningmastery.com/super-learner-ensemble-in-python/).
