...
- [Skip to the end](#the-proof) to see the proof.
...
I soon realized that for what I needed to do, my grouping variable should not be in the regression model. However, I was still perplexed as to how the adjusted and unadjusted group means could be the same after adjusting for other variables.
...
$=E[E[Y|X,Z=z]|Z=z]$ ^[Because we're making our outer expectation conditional on $Z=z$, we can also move $Z=z$ into our inner expectation.]
...
$=\sum_{Y}y\sum_{X}\frac{P(Y=y,X=x,Z=z)}{P(Z=z)}$ ^[We can switch the summations around so that $y$ is outside the summation. This lets us get the joint distribution of only $Y$ and $Z$.]
