...
Fair warning: this step is easy to code, but difficult to understand unless you have a background in semiparametric theory. Since this post is focused on learning the TMLE algorithm, I'll give only a very brief high-level explanation, and then you can look at references if you're curious to learn more about the theory. Importantly, **the explanation I offer for this step is sufficient to appropriately implement TMLE as an analyst.**

In this step, we will construct an estimating equation that happens to solve for something called the **Efficient Influence Function** (EIF) of the ATE. An Influence Function (IF) is a function that tells us how much an estimate will change if the input changes. If an Efficient IF exists for an estimand, it means the estimand can be estimated **efficiently** (achieving the smallest possible variance in the fewest number of observations).
...
Thank you to Iván Díaz for patiently answering my many, many questions on TMLE, and to Miguel Luque for helpful feedback on the visual guide.
