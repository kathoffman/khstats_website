...
*Note that a TMLE estimate is asymptotically normal, so we could bootstrap the entire algorithm to get our standard errors instead.*
...
2. Some statistical estimands have an **efficient influence function** (EIF), which means there exists an influence function that achieves the efficiency bound (think Cramer Rao Lower Bound from parametric maximum likelihood estimation) and can be used to create efficient estimators.
...
I could only cover so much in this post, so I'm sure you'll have plenty of questions! I've used the most while learning about TMLE and other semiparametric estimation methods:
