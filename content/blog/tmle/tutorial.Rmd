...
In this step, we will construct an estimating equation to help us find something called the Efficient Influence Function (EIF) of the ATE. An **influence function** is a function that quantifies how much an estimate will change if the input data changes. Some estimands have an **efficient influence function**, which means there exists an influence function that achieves the Cramer Rao Lower Bound and can be used to create **efficient** estimates. (Informally, an efficient estimate means it achieves the smallest possible variance in the fewest number of observations.)

<!-- *Note: We don't call it an EIF in MLE because the score function **always** achieves the Cramer Rao Lower Bound of efficiency. It's not quite as simple in a semi-parametric space.* -->

If we were to work for a mathematical proof, we could show the ATE has an EIF, and that there is an equation we can set up to *target* it. **Setting up this estimating equation for the EIF will allow us to do two things:**

  1. Update our initial outcome estimates so that our estimate of the ATE is asymptotically unbiased (under certain conditions, see the [statistical properties section](#7-properties-of-tmle)).

  2. Construct standard errors, confidence intervals, p-values, etc. for our estimate of the ATE.

**This is the reason TMLE allows us to use machine learning models "under the hood" while still obtaining asymptotic properties for inference: our estimand of interest has an EIF, and we've built an algorithm, or estimator, to utilize its beneficial properties.**
...
Do you see how our estimating equation *also* has a vector on the right-hand side, $H(A,\mathbf{W})$, with a corresponding coefficient, $\epsilon$? The only difference is that the "intercept" in our estimating equation, $logit(\mathrm{\hat{E}}[Y|A,\mathbf{W}])$ is not a constant value like $\beta_0$; it is a vector of values. We can see it as an **offset** or a **fixed intercept** in a logistic regression, rather than a constant-value intercept.
