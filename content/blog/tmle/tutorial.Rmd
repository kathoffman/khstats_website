...
> A beginner's guide to understanding Targeted Maximum Likelihood Estimation (TMLE) via a step-by-step tutorial for estimating the mean difference in an outcome, adjusting for confounders. In causal inference, this is called the Average Treatment Effect (ATE) when causal assumptions are met.
...
1. If the goal is <span style="color: #3366ff;">inference</span> (for example, an effect size with a confidence interval), use an <span style="color: #3366ff;">interpretable, usually parametric, model</span> and explain the meaning behind the coefficients and their standard errors. 

2. If the goal is  <span style="color: #cc0000;">prediction</span>, use <span style="color: #cc0000;">flexible non- and semi-parametric machine learning algorithms</span> and then look at performance metrics and variable importance.
...
*Note that the choice of regressions for superlearning is not the point of this post; we simply want to add flexible machine learning models which we think <u>**may**</u> reflect the underlying data distribution. A penalized regression such as LASSO may not be necessary with only four covariates, but in general this is a good candidate learner to try.*
...
- Then, in Step 3, we used quantities from Step 1 and Step 2 to solve an estimating equation for the EIF. We didn't talk about *why* we have to do this because it is impossible to explain correctly to beginners without distracting from *what* we're doing.
...
Lastly, many thanks to Nick Williams, Axel Martin, Anjile An, Adam Peterson, Alan Wu, and Will Simmons for providing suggestions on various drafts of this project!
