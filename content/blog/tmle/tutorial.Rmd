---
title: "An Illustrated Introduction to Targeted Maximum Likelihood Estimation (TMLE)"
author: "Katherine Hoffman"
date: 2020-11-09T21:13:14-05:00
...
Second, I thought flexible, data-adaptive models we commonly classify as *machine learning* could only be used for prediction, since they don't have asymptotic properties for inference (i.e. standard errors). However, certain semiparametric estimation methods like TMLE can actually use these models to obtain a final estimate that is closer to the target quantity than would be obtained using standard parametric models (e.g. linear and logistic regression). This is because machine learning models can often accommodate large numbers of confounders with complex relationships much better than parametric models.
...
🚨 Warning: this step is easy to code, but difficult to understand unless you have a background in semiparametric theory. Since this post is focused on learning the TMLE algorithm, I'll give only a very brief high-level explanation, and then you can look at references if you're curious to learn more about the theory. Importantly, the explanation I offer for this step is more than sufficient to appropriately implement TMLE as an analyst.


> The point of this step is to set up an estimating equation for the *efficient influence function* of our estimand of interest.

An **influence function** is a function that quantifies how much influence each observation has on the estimator. For this reason, it is very useful to characterize the variance of the estimator. Some estimands have an **efficient influence function** (EIF), which means there exists an influence function that achieves the Cramer Rao Lower Bound and can be used to create efficient estimators. (Informally, an efficient estimator means it achieves the smallest possible variance for a fixed number of observations.)

If we were to work through a mathematical proof, we could show the ATE has an EIF, and that there is an equation we can set up to *target* it. **Setting up this estimating equation for the EIF will allow us to do two things:**
...
This point estimate is great, but usually standard errors are needed to compute confidence intervals and test statistics. To obtain the standard errors, we first need to compute the **Influence Curve** (IC), which is the empirical version of what we used our estimating equation to figure out in step 3. The IC tells us how much each observation influences the final estimate. 
...
Once we have the IC, we can rely on the delta method by taking the square-root of its variance divided by the number of observations to get the standard error of our estimate.

*Note: This is another step which relies on semiparametric theory so I won't get into the details; however, I've linked resources in [References](#references).*
...
*Note: A TMLE estimate is asymptotically normal, so we could bootstrap the entire algorithm to get our standard errors instead.*
...
A huge thank you to Iván Díaz for patiently answering many, many questions on TMLE. I am  also very appreciative of Miguel Luque, Nick Williams, Axel Martin, and Anjile An for their very helpful feedback on this tutorial.
