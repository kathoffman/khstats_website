---
title: "My favorite lines of data wrangling code during the NYC COVID-19 outbreak"
...
In non-coronavirus times, I am the biostatistician for a team of NYC pulmonologists and intensivists. When the pandemic hit NYC in mid-March, I immediately became a <strike>100%</strike> 200% COVID-19 statistician. I received many analysis requests, though not all of them from official PIs:
...
Jokes aside, I was really busy during the outbreak. While doing work for both hospital operations and rapidly moving COVID-19 academic research, it was more important than ever to code efficiently and accurately while working with a deluge of Electronic Health Record (EHR) data.

This post contains my favorite (because they were either the most useful or most used) lines of `R` code for turning the very raw form EHR data into analytical files for descriptive reporting and statistical inference. For the examples, I've created data that is a simplified version of the actual data I used during the NYC outbreak -- just in case you too would like to pretend you are a COVID-19 biostatistician!
...
Perfect! I could go on to informatively join this data to another using `tidylog`, make tables with clean labels using `tbl_summary()`, `set_variable_labels()`, and `snakecase()`, find further labs of interest with `filter()`, `str_detect()`, and `tolower()`, and make the lab values numeric with `parse_number()`... the possibilities are endless!
