...
I've since received requests to **add additional variables to the timelines**. This post shows how to do that.

I'll use a toy dataset on hospitalized COVID-19 patients, available to download on this [Github repository](https://github.com/kathoffman/steroids-trial-emulation). It is derived from a dataset from Electronic Health Record data Spring 2020. This is a time period when there was large variation in provider practice in administering steroids, a type of drug that combats hyper-inflammation. Steroids are usually given to patients which exhibit an inflammtory profile; we will identify this using a threshold for low oxygen levels (severe hypoxia).
