...
dat <- 
  ...
  mutate(pt_id = factor(row_number()),  # patient ids are 1-n
         ...
         death = rbinom(n, 1, .5)) %>% # death is not affected by treatment and randomly occurs at any time point. Obviously an unrealistic assumption. :)
...
```{r}
dat %>% filter(pt_id == 5) %>% knitr::kable()
```
